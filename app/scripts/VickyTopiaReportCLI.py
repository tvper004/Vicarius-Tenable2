...
from TenableClient import TenableClient
import gc
...
def get_config(key, secret_name=None, default=None):
    """
    Retrieves configuration from Environment Variable first, then Docker Secret.
    """
    ...
    # 2. Check Docker Secret (if secret_name provided)
    if secret_name:
        try:
...
def read_secret(secret_name):
    ...
    return get_config(secret_name.upper(), secret_name)
...
parser.add_argument('--tenableReport', dest='tenableReport', action='store_true', help='Tenable Reports')
...
apikey = get_config('VICARIUS_API_KEY', 'api_key')
organization_domain = get_config('VICARIUS_DASHBOARD_ID', 'dashboard_id')
urldashboard = f"https://{organization_domain}.vicarius.cloud"

# Tenable Credentials (Placeholder for future implementation)
tenable_api_key = get_config('TENABLE_API_KEY')
tenable_secret_key = get_config('TENABLE_SECRET_KEY')

#Initialization Postgresql
host = "appdb"
port = "5432"
user = get_config('POSTGRES_USER', 'postgres_user')
password = get_config('POSTGRES_PASSWORD', 'postgres_password')
database = get_config('POSTGRES_DB', 'postgres_db')
optionalTools = get_config('OPTIONAL_TOOLS', 'optional_tools', default="")
...
def main():
    ...
    if vRxSetup == 0:
        ...
        elif args.tenableReport:
            reports = "tenableReport"
            print(reports)
            try:
                ReportTenable()
            except Exception as e:
                errorList.append("ReportTenable:" + str(e))
                print(str(e)) 
...
def ReportTenable():
    print("Starting Tenable Report...")
    
    # Check if keys are present
    if not tenable_api_key or not tenable_secret_key:
        print("Error: Tenable API Key or Secret Key not found in configuration.")
        return

    # Check/Create Tables
    db.check_create_table_tenable_assets(host, port, user, password, database)
    db.check_create_table_tenable_vulnerabilities(host, port, user, password, database)

    client = TenableClient(tenable_api_key, tenable_secret_key)

    # 1. Fetch Assets
    print("Fetching Tenable Assets...")
    assets_data = client.get_assets()
    print(f"Found {len(assets_data)} assets.")
    if assets_data:
        db.insert_into_table_tenable_assets(assets_data, host, port, user, password, database)

    # 2. Fetch Vulnerabilities
    print("Fetching Tenable Vulnerabilities...")
    vulns_data = client.export_vulns() # Using the iterative approach 
    print(f"Found {len(vulns_data)} vulnerabilities.")
    if vulns_data:
        db.insert_into_table_tenable_vulnerabilities(vulns_data, host, port, user, password, database)

    # 3. Create/Update Unified View
    print("Updating Unified Assets View...")
    db.create_view_unified_assets(host, port, user, password, database)

    print("Tenable Report Completed.")
